 ...
 */
public class RecorderQueueList<T> {
    ...
    private ConcurrentLinkedQueue<Sample<T>> queue = new ConcurrentLinkedQueue<>();
    private boolean isDone = false;

    public RecorderQueueList() {
    ...
    public boolean add(T x) {
        ...
        while (true) {
            ...
            if(found){
                queue.add(new Sample<T>(1, x, false));
                return false;
            } else {
                ...
                if(pred.next[bottomLevel].compareAndSet(succ, newNode, false, false))
                    queue.add(new Sample<T>(1, x, true));
                else
                    continue;
    ...
    public boolean populate(T x) {
        ...
        while (true) {
            ...
            if (found) {
                ...
                Node<T> pred = preds[bottomLevel];
                Node<T> succ = succs[bottomLevel];
                boolean compareAndSetSuccess;
                compareAndSetSuccess = pred.next[bottomLevel].compareAndSet(succ, newNode, false, false);
                if (!compareAndSetSuccess) {
                    continue;
                }
                for (int level = bottomLevel+1; level <= topLevel; level++) {
                    while (true) {
    ...
    public boolean remove(T x) {
        ...
        while (true) {
            boolean found = find(x, preds, succs);
            if(!found) {
                queue.add(new Sample<T>(2, x, false));
                return false;
            } else {
                ...
                while (true) {
                    ...
                    if (iMarkedIt)
                        queue.add(new Sample<T>(2, x, true));
    ...
    public boolean contains(T x) {
        ...
        queue.add(new Sample<T>(linearizationTime, 0, x, (curr.key == v)));
        return (curr.key == v);
    }
    ...
    public void queueMaster() {
        while (!isDone || !queue.isEmpty()) {
            Sample<T> s = queue.poll();
            if(s != null)
